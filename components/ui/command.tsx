...
import { useCommandContext } from "../health-care/custom-command-search";
...
>(({ className, ...props }, ref) => (
  <div
    className="flex items-center border-b px-3 border-[#363A3D]"
    cmdk-input-wrapper=""
  >
...
>(({ children, className, onSelect, value, ...props }, ref) => {
  const { onClose, selectedValue } = useCommandContext();
  return (
...
const CommandShortcut = ({
  ...
  return (
    <span
      className={cn(
        "ms-auto text-xs tracking-widest text-neutral-500 dark:text-neutral-400",
        className
      )}
      {...props}
    />
